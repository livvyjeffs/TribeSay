...
if (isset($_GET['id']) && isset($_GET['u']) && isset($_GET['e']) && isset($_GET['p'])){
    ...
    if($id == "" || strlen($u) < 3 || strlen($e) < 5 || strlen($p) == ""){
        //log this issue into a text file and emaili the details to myself...
        header("location: message.php?msg=activation_string_length_issues");
        exit();
    }
    //Check their credentials against the database
    $sql = "SELECT * FROM users WHERE id='$id' AND username='$u' AND email='$e'
            AND password='$p' LIMIT 1";
    $query= mysqli_query($db_conx, $sql);
    $numrows = mysqli_num_rows($query);
    //Evaluate for a match in the system (0 = no match, 1 = match)
    if($numrows === 0){
        //Log this potential hack attempt to a text file and email details to myself
        header("location: message.php?msg=Your credentials are not matching 
    ...
    $query = mysqli_query($db_conx, $sql);
    //Optional double check to see if activated in face now = 1
