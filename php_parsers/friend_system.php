...
if (isset($_POST['type']) && isset($_POST['user'])){
	...
	if($_POST['type'] === "follow"){/*
	    ...
	    if($friend_count[0] > 99){
	        ...
	        $sql = "INSERT INTO friends(user1, user2, datemade, accepted) VALUES('$log_username','$user',now(), '1')";
		    $query = mysqli_query($db_conx, $sql);
			mysqli_close($db_conx);
...
?><?php
   /* if(isset($_POST['action']) && isset($_POST['reqid']) && isset($_POST['user1'])){
        ...
        if($_POST['action'] === "accept"){
            ...
            $query = mysqli_query($db_conx, $sql);
            $row_count1 = mysqli_fetch_row($query);
            $sql = "SELECT COUNT(id) FROM friends WHERE user1='$user' AND user2='$log_username' AND accpeted='1' LIMIT 1";
            $query = mysqli_query($db_conx, $sql);
            $row_count2 = mysqli_fetch_row($query);
            if($row_count[0] > 0 || $row_count2[0] > 0){
                mysqli_close($db_conx);
                echo "You are already friends with $user.";
                exit();
            } else{
