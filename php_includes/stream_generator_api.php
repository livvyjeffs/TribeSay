...
//stream media type
if (isset($_POST['stream_media_type'])) {
    $stream_media_type = $_POST['stream_media_type'];
} elseif (isset($_GET["stream_media_type"])) {
    $stream_media_type = $_GET['stream_media_type'];
}
//current id array
if (isset($_POST['current_id_list'])) {
    $current_id_array = $_POST['current_id_list'];
} elseif (isset($_GET["current_id_list"])) {
    $current_id_array = $_GET['current_id_list'];
}
//scope
...
} elseif (isset($_GET["infinite"])) {
    $infinite = $_GET['infinite'];
}
//splode_status
if (isset($_POST['splode_status'])) {
    $splode_status = $_POST['splode_status'];
} elseif (isset($_GET["splode_status"])) {
    $splode_status = $_GET['splode_status'];
}
//trigger
...
//page_owner
if (isset($_POST['page_owner'])) {
    $page_owner = $_POST['page_owner'];
} elseif (isset($_GET["page_owner"])) {
    $page_owner = $_GET['page_owner'];
}
...
if (isset($scope) && isset($current_id_array) && isset($page_owner) && isset($splode_status)) {
    ...
    if ($current_id_array !== "non_purge") {
        $total_count = 0;
        $current_id_array = explode(",", $current_id_array);
        if($trigger !== "fresh_load"){
    ...
    } elseif (isset($app_filter_array)) {
        $selectedFavsArray = $app_filter_array;
    } else {
    ...
    foreach ($databases as $type => $base_of_data) {
        ...
        if ($type === "event") {
            $order_by = "event_begin";
        } else {
            $order_by = "postdate";
        }
        switch ($scope) {
            case "single":
                $sql = "SELECT * FROM " . $base_of_data . " WHERE poster='$page_owner'" . $criteria . $event_criteria. " ORDER BY ".$order_by." DESC LIMIT ".$start_count.",40";
            ...
            case "tribe":
                ...
                $sql = "SELECT * FROM " . $base_of_data . $where . substr($criteria, 4). $event_criteria. " ORDER BY ".$order_by." DESC LIMIT ".$start_count.",40";
                $sql2 = "";
                break;
        }
    ...
    foreach ($databases as $type => $base_of_data) {
            ...
            while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
                ...
                switch ($type) {
                    ...
                    case "event":
                        ...
                        $content_unit["event_begin"] = $row["event_begin"];
                        $content_unit["event_end"] = $row["event_end"];
                        $content_unit["campaign_begin"] = $row["campaign_end"];
        ...
        // sort alphabetically by name
            ...
            usort($$type, 'compare_score');//may need a custom compare function for events...
        }else{
